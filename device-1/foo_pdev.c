...
static void foo_platform_device_release(struct device *dev)
{
}
...
struct platform_device foo_platform_device = {                                   
	.name = "foo",
	.id = -1,
	.dev = { 
		.groups = foo_groups,
		.release = foo_platform_device_release,
	},
	.num_resources = ARRAY_SIZE(foo_resource),
	.resource = foo_resource,
};                                                                              
EXPORT_SYMBOL(foo_platform_device);
...
static int __init foo1_init(void)
{
	...
	ret = platform_device_register(&foo_platform_device);
	if (ret < 0) {
	...
	foo0_dev = &foo_platform_device.dev;
...
{
	platform_device_unregister(&foo_platform_device);
