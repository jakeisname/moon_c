...
static irqreturn_t virtual_gpio_interrupt (int irq, void *data)
{
    ...
    printk(KERN_INFO "%s irq=%d, pending=0x%lx\n", __func__, irq, pending);
    ...
    iowrite32(0xffffffff, vg->data_base_addr + VIRTUAL_GPIO_INT_EOI);

    return IRQ_WAKE_THREAD;
}

static irqreturn_t virtual_gpio_thread(int irq, void *data)
{
